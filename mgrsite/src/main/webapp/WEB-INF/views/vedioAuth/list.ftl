...
<script type="text/javascript">
	$(function() {
		...
		$("#loginInfoDisplay").typeahead({
			...
			source:function(query,process){//输入最小查询字符后加载数据源
				//我们使用ajax去查询
				$.ajax({
					dataType:"json",
					type:"POST",
					url:"vedioAuth_autocomplate.do",
					data:{keyword:query},//输入的数据封装在query中,发送请求时则将keyword的值query发送到controller中
					success:function(data){//这个data就是一个json对象的数组{id:xx,username:xxxx}
						if(data && data.length){
							process(data);//process回调函数就是交给我们获得数据之后用来调用的方法,这个方法执行了,下拉列表就出来了
						}
					}
				});
			},
			//复写displayText方法，需要显示对象中哪一个字段的内容
			//用来告诉typeahead怎么显示json对象中的内容
			displayText:function(item){
				return item.username
			}
		}).change(function(){//当自动补全后选中相关数据之后，获取选中的数据
			var current = $(this).typeahead("getActive");
			if(current){
				//将获取的选中数据放入hidden域中方便提交时识别审核的是哪一个用户
