...
<script type="text/javascript">
	$(function() {
		...
		$(".auditClass").click(function () {
			 ...
			 //回显数据
			 $("#id").val(json.id);
			 $("#username").html(json.username);//label使用html不能使用val回显
		...
		$(".btn_audit").click(function () {
			  //将提交审核的值赋值给hidden中的state状态值，用于确认时审核通过还是失败
		      $("#state").val($(this).val())
			$("#editform").ajaxSubmit({
				dataType:'json',
				success:function (data) {
					if(data.success){ $.messager.confirm("提示","操作成功",function () {
			...
			<div class="col-sm-9">
				...
				<form id="searchForm" class="form-inline" method="post" action="/realAuth.do">
					...
					<div class="form-group">
					    ...
					    <input class="form-control" type="text" name="endDate" id="endDate" value='${(qo.beginDate?string("yyyy-MM-dd"))!""}' />
					</div>
					<div class="form-group">
