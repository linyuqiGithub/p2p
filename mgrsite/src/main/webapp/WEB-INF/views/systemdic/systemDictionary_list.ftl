...
<head>
	...
	$(function(){
		...
		$("#addSystemDictionaryBtn").click(function () {
			//拿到editForm的JQuery对象(伪数组)的第一个数DOM对象，调用表单DOM对象的reset方法,将表单的数据进行重置
		    $("#editForm")[0].reset();
		    //清空隐藏域(DOM对象的reset方法不会对隐藏域进行清空，我们需要手动清空隐藏域的值)
