		...
		<div class="row">
			...
			<div class="col-sm-9">
				...
				<form id="searchForm" class="form-inline" method="post" action="/rechargeOffline.do">
					...
					<div class="form-group">
					    <label>开户行</label>
					    <select class="form-control" name="bankinfo">
					    	<option value="-1">全部</option>
					    	<#list banks as bank>
	    						<option value="${bank.id}">
									<script>
										var str="("+SITE_BANK_TYPE_NAME_MAP["${bank.bankName}"]+")${bank.bankNumber}${bank.forkName}";
										document.write(str);
									</script>
								</option>
	    					</#list>
					    ...
					    <script type="text/javascript">
					    	$("[name=bankinfo] option[value='${(qo.bankinfo)!''}']").attr("selected","selected");
					    </script>
					</div>
					<div class="form-group">
					    <label>交易时间</label>
					...
					<table class="table">
						...
						<#list result.data as info>
							<tr>
								...
								<td>
									<script>
										var str="("+SITE_BANK_TYPE_NAME_MAP["${info.bankinfo.bankName}"]+")${info.bankinfo.bankNumber}${info.bankinfo.forkName}";
										document.write(str);
									</script>
								</td>
