package com.xmg.p2p.bussiness.util;

import com.xmg.p2p.base.util.Consts;
import com.xmg.p2p.base.util.DecimalFormatUtil;
...
public class CalculatetUtil {
	...
	public static BigDecimal getMonthlyRate(BigDecimal yearRate) {
		...
		return yearRate.divide(ONE_HUNDRED).divide(NUMBER_MONTHS_OF_YEAR, Consts.CALCULATE_SCALE, RoundingMode.HALF_UP);
	}
	...
	public static BigDecimal calTotalInterest(int returnType, BigDecimal bidRequestAmount, BigDecimal yearRate,
		...
		if (returnType == Consts.RETURN_TYPE_MONTH_INTEREST_PRINCIPAL) {// 按月分期
			// 只借款一个月
			if (monthes2Return == 1) {
				totalInterest = bidRequestAmount.multiply(monthlyRate).setScale(Consts.CALCULATE_SCALE, RoundingMode.HALF_UP);
			} else {
				...
				// 算出每月还款
				BigDecimal monthToReturnMoney = temp1.multiply(temp2).divide(temp3, Consts.CALCULATE_SCALE,
	...
	public static BigDecimal calMonthlyInterest(int returnType, BigDecimal bidRequestAmount, BigDecimal yearRate,
		...
		if (returnType == Consts.RETURN_TYPE_MONTH_INTEREST_PRINCIPAL) {// 按月分期
			// 只借款一个月
			if (monthes2Return == 1) {
				monthlyInterest = bidRequestAmount.multiply(monthlyRate).setScale(Consts.CALCULATE_SCALE,
						RoundingMode.HALF_UP);
			} else {
				BigDecimal temp1 = bidRequestAmount.multiply(monthlyRate);
				BigDecimal temp2 = (BigDecimal.ONE.add(monthlyRate)).pow(monthes2Return);
				...
				if (monthIndex < monthes2Return) {
					monthlyInterest = DecimalFormatUtil.amountFormat((temp1.subtract(monthToReturnMoney))
							.multiply(temp4).add(monthToReturnMoney).setScale(Consts.CALCULATE_SCALE, RoundingMode.HALF_UP));
					...
					for (int i = 1; i < monthes2Return; i++) {
						...
						monthlyInterest = (temp1.subtract(monthToReturnMoney)).multiply(temp5).add(monthToReturnMoney)
								.setScale(Consts.CALCULATE_SCALE, RoundingMode.HALF_UP);
	...
	public static BigDecimal calMonthToReturnMoney(int returnType, BigDecimal bidRequestAmount, BigDecimal yearRate,
		...
		if (returnType == Consts.RETURN_TYPE_MONTH_INTEREST_PRINCIPAL) {// 按月分期
			if (monthes2Return == 1) {
				...
				BigDecimal temp3 = (BigDecimal.ONE.add(monthlyRate)).pow(monthes2Return).subtract(BigDecimal.ONE);
				// 算出每月还款
				monthToReturnMoney = temp1.multiply(temp2).divide(temp3, Consts.CALCULATE_SCALE, RoundingMode.HALF_UP);
			}
		} else if (returnType == Consts.RETURN_TYPE_MONTH_INTEREST) {// 按月到期
			BigDecimal monthlyInterest = bidRequestAmount.multiply(monthlyRate).setScale(Consts.CALCULATE_SCALE,
					RoundingMode.HALF_UP);
			if (monthIndex == monthes2Return) {
				monthToReturnMoney = bidRequestAmount.add(monthlyInterest).setScale(Consts.CALCULATE_SCALE,
						RoundingMode.HALF_UP);
			} else if (monthIndex < monthes2Return) {
	...
	public static BigDecimal calBidInterest(BigDecimal bidRequestAmount, int monthes2Return, BigDecimal yearRate,
		...
		// 所占比例
		BigDecimal proportion = acturalBidAmount.divide(bidRequestAmount, Consts.CALCULATE_SCALE, RoundingMode.HALF_UP);
	...
	public static BigDecimal calInterestManagerCharge(BigDecimal interest) {
		return DecimalFormatUtil.formatBigDecimal(interest.multiply(INTEREST_MANAGER_CHARGE_RATE), Consts.STORE_SCALE);
	}
